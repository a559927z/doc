        ...
        getRequestData: function(organId, time) {
        	...
        	$.get(urls.queryOvertimeByOrganUrl, 
        		...
        		$.each(data.list, function (i, item) {
            		 ...
            		 self.data[0].data.push(parseFloat(item.avgNum).toFixed(2));
            		 self.data[1].data.push(parseFloat(item.totalNum).toFixed(2));
                });
        ...
        packMarkLineData: function (cateLen, value) {
            ...
            $("#"+self.btnId).children().on(clickEvent,function(){
        	 ...
        	 if(self.xAxisData.length>0){
        		 self.option.xAxis[0].data = self.xAxisData;
                 self.option.series[0].data = self.data[type].data;
