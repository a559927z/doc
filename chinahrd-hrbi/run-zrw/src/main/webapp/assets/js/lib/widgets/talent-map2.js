define(['jquery', 'template', 'zrender', 'zrender/shape/Polyline', 'zrender/shape/Polygon', 'zrender/shape/Text',
    ...
    $.fn.talentMap = function (options) {
        var map = {
                ...
                if (m._groupMode) {
                    ...
                    for (var x = 1; x <= xlen; x++) {
                        for (var y = 1; y <= ylen; y++) {
                            var cell = m._findCell(x, y);
                            if (!p.showTeam) {
                                m._addGroupPointHtml(cell, countMap[x + '_' + y], x, y, p.teamColors[0]);
                                continue;
                            }
                            for (var t = 0; t < tlen; t++) {
                                m._addGroupPointHtml(cell, countMap[x + '_' + y + '_' + t], x, y, p.teamColors[t]);
                            }
                        }
                    }
                } else {
            ...
            _addGroupPointHtml: function (cell, cnt, x, y, color) {
                // 等于0就不显示
                if (cnt) {
                    var m = this, p = m.options;
