...
//            "<div class='month'><%= month %> <%= year %></div>" +
        ...
        "<table class='clndr-table' border='0' cellspacing='0' cellpadding='0'>" +
            ...
            "<% for(var i = 0; i < numberOfRows; i++){ %>" +
                ...
                "<% for(var j = 0; j < 7; j++){ %>" +
                "<% var d = j + i * 7; %>" +
                	"<% if((days[d].date._d.getFullYear() == sysYear && days[d].date._d.getMonth() == sysMonth && days[d].day < sysDay) || (days[d].date._d.getFullYear() == sysYear && days[d].date._d.getMonth() < sysMonth) || days[d].date._d.getFullYear() < sysYear) { %>" +
	                	"<% if(attdanceDay.t2.indexOf(days[d].time) != -1 && days[d].date._d.getMonth() == curMonth) { %>" +
	                    	"<td class='<%= days[d].classes %> red-background curs'>" +
	                    "<% } else { %>" +
	                    	"<td class='<%= days[d].classes %> curs'>" +
	                    "<% } %>" +
	                "<% } else { %>" +
	                	"<td class='<%= days[d].classes %>'>" +
	                "<% } %>" +
	                "<% if(sysYear == days[d].date._d.getFullYear()) { %>" +
	                	"<% if(days[d].date._d.getMonth() < sysMonth) { %>" +
                			"<% if(attdanceDay.alldays.indexOf(days[d].time) != -1 && days[d].date._d.getMonth() == curMonth) { %>" +
                    			"<% for(var k = 0; k < attdance.length; k++) { %>" +
			                    	"<% if(attdance[k].day == days[d].time && attdance[k].id == 0) { %>" +
		                				"<div class='day-contents'><%= days[d].day %></div><div class='normal-contents'></div>" +
		                			"<% } else if(attdance[k].day == days[d].time && attdance[k].id == 1) { %>" +
		                				"<% if(allTheory.indexOf(days[d].time) == -1) { %>" +
				                			"<div class='day-contents vacation'><%= days[d].day %></div><div class='day-over-time'><%= attdance[k].typeName %></div>" +
			                			"<% } else { %>" +
			                				"<div class='day-contents'><%= days[d].day %></div><div class='day-over-time'><%= attdance[k].typeName %></div>" +
			                			"<% } %>" +
		                			"<% } else if(attdance[k].day == days[d].time && attdance[k].id == 2) { %>" +
		                				"<div class='day-contents'><%= days[d].day %></div><div class='exception-contents'></div>" +
		                			"<% } %>" +
		                		"<% } %>" +
	                		"<% } else if(attdanceDay.alldays.indexOf(days[d].time) == -1 && days[d].date._d.getMonth() == curMonth) { %>" +
		                		"<% if(allTheory.indexOf(days[d].time) == -1) { %>" +
		                			"<div class='day-contents vacation'><%= days[d].day %></div>" +
	                			"<% } else { %>" +
	                				"<div class='day-contents'><%= days[d].day %></div>" +
	                			"<% } %>" +
                			"<% } else { %>" +
	            				"<div class='day-discontents'><%= days[d].day %></div>" +
	            			"<% } %>" +
	                    "<% } else if(days[d].date._d.getMonth() == sysMonth) { %>" +
	                    	"<% if(attdanceDay.alldays.indexOf(days[d].time) != -1 && days[d].date._d.getMonth() == curMonth) { %>" +
		            			"<% for(var k = 0; k < attdance.length; k++) { %>" +
			                    	"<% if(attdance[k].day == days[d].time && attdance[k].id == 0) { %>" +
		                				"<div class='day-contents'><%= days[d].day %></div><div class='normal-contents'></div>" +
...
//		                				"<div class='day-discontents'><%= days[d].day %></div>" +
		                			"<% } %>" +
		                		"<% } %>" +
		            		"<% } else if(attdanceDay.alldays.indexOf(days[d].time) == -1 && days[d].date._d.getMonth() == curMonth && days[d].day < sysDay) { %>" +
			            		"<% if(allTheory.indexOf(days[d].time) == -1) { %>" +
		                			"<div class='day-contents vacation'><%= days[d].day %></div>" +
		            			"<% } else { %>" +
		            				"<div class='day-contents'><%= days[d].day %></div>" +
	                    ...
	                    "<% } else { %>" +
            				"<div class='day-discontents'><%= days[d].day %></div>" +
            			"<% } %>" +
                    "<% } else if(sysYear > days[d].date._d.getFullYear()) { %>" +
	                    "<% if(attdanceDay.alldays.indexOf(days[d].time) != -1 && days[d].date._d.getMonth() == curMonth) { %>" +
		        			"<% for(var k = 0; k < attdance.length; k++) { %>" +
		                    	"<% if(attdance[k].day == days[d].time && attdance[k].id == 0) { %>" +
		            				"<div class='day-contents'><%= days[d].day %></div><div class='normal-contents'></div>" +
    ...
    function Clndr(element, options) {
        ...
        // current month as the time period.
        	...
        	if(null != this.options.curDate) {
        		this.month = moment(this.options.curDate);
	            this.intervalStart = moment(this.options.curDate);
	            this.intervalEnd = moment(this.options.curDate).endOf('month');
        	} else {
	        	var now = new Date();
	            var nowDay = now.getDate();
	            if(nowDay <= this.options.warnDay) {
	            	this.month = moment().subtract(1, 'M');
    ...
    Clndr.prototype.init = function () {
        ...
        if (!this.options.daysOfTheWeek) {
            this.daysOfTheWeek = ['日', '一', '二', '三', '四', '五', '六'];
...
//            }
    ...
    Clndr.prototype.render = function () {
        ...
        if (this.options.lengthOfTime.days) {
            ...
            var curMonth = currentMonth._d.getMonth();
            var dd = new Date();
            var sysDay = dd.getDate();
            var sysMonth = dd.getMonth();
            var sysYear = dd.getFullYear();
            
            var attdance = this.options.attdance;
            var attdanceDay = this.options.attdanceDay;
            var allTheory = this.options.allTheory;
            ...
            data = {
                ...
                date: this.month.format('YYYY年MM月'),
                daysOfTheWeek: this.daysOfTheWeek,
                eventsLastMonth: this.eventsLastMonth,
                eventsNextMonth: this.eventsNextMonth,
                numberOfRows: Math.ceil(days.length / 7),
