 ...
 */
require(['jquery', 'jquery-ui', 'echarts', 'zrender/shape/Text', 'echarts/chart/bar', 'echarts/chart/line', 'echarts/chart/pie', 'echarts/chart/gauge',
    'bootstrap', 'jgGrid', 'underscore', 'timeLine2', 'messenger', 'selection', 'select2', 'organTreeSelector'], function ($, jqueryui, echarts, TextShape) {
    ...
    //提示
    $("#applicationSomeConformityNum .icon-question-sign").attr("title", "说明：<br><li>符合条件" + $("#conditionProp").val() + "%以上人员</li><li>包含申请和未申请人员</li><li>不包含符合条件人员</li>").tooltip({
        placement: 'bottom',
        html: true
    });
...
//		    		teamRankingObj.setOption();
    ...
    var promoteTrackObj = {
        ...
        renderChart: function () {
            ...
            $.each(self.requestIds, function (i, item) {
            	...
            	empIdArr.push(vra);
                arr.push('<span class="item unselectable" style="border: 1px solid ' +colorArr[i] + ';color: ' + colorArr[i] + ';">' + item.k + '<i data-id="' + item.v + '" class="icon-remove-sign" title="删除"></i></span>');
            });
            ...
            var empIds = [];
            $.each(data, function (i, item) {
                if (!_.contains(empIds, item.empId)) {
        ...
        resizeGrid: function () {
            var self = this;
            if (self.jqGrid) {
                self.jqGrid.setGridWidth($("#promoteTrack").width());
            }
        }
    }
    ...
    var promoteStatusObj = {
            ...
            colModel: [
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        ...
                        var arr = [];
                        arr.push('<div class="prostatusinp clearfix">');
                        arr.push('	<div class="prostatusimg"><img src="' + imgPath +'" data="' + rowObject.empId + '"  class="talent_col" id="renImg"></div>');
                        arr.push('	<div class="prostatusnp">');
                        arr.push('		<div class="prostatusname">' + rowObject.userName + '</div>');
                        arr.push('		<div class="prostatuspro">' + rowObject.organizationName + '/' + rowObject.rankName + '</div>');
                        arr.push('	</div>');
                        arr.push('</div>');
                        return arr.join('');
                    }
                },
                {
                    ...
                    align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
                        return rowObject.rankNameNext + "<br>(" + (rowObject.status > 0 ? "已申请" : "未申请") + ")";
                    }
                },
                {
                    name: 'condition',
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        ...
                        if (rowObject.listReq.length > 0) {
                            arr.push('<table class="prostatustb">');
                            $.each(rowObject.listReq, function (i, item) {
                                arr.push('  <tr>');
                                arr.push('    <td class="left">' + item.note);
                                switch (item.noteIndex) {
                                    case 0: {
                                        arr.push('</br>（'+item.promotionRequest + '年）</td>');
                                        arr.push('    <td class="center">' + item.persomaStatus + '年</td>');
                                        break;
                                    }
                                    case 1: {
                                        ...
                                        arr.push('</br>（' + s.join('') + '）</td>');
                                        arr.push('    <td class="center">' + item.persomaStatus + '</td>');
                                        break;
                                    }
                                    default: {
                                        arr.push('</br>（' + item.promotionRequest + '）</td>');
                                        arr.push('    <td class="center">' + item.persomaStatus + '</td>');
                                        break;
                                    }
                                }
                                arr.push('    <td class="center">' + (item.isAccord > 0 ? '<i class="icon-ok"></i>' : '<i class="icon-remove"></i>') + '</td>');
                                arr.push('  </tr>');
                            });
                            arr.push('</table>');
                        }
        ...
        event: function () {
            ...
            $("#promoteStatusSearchMore").unbind("click").on("click", function () {
                ...
                if (conditionIndex > 0 && conditionBegin == "") {
                    $("#conditionBegin").focus();
                    return;
                }
                else if (conditionIndex > 0 && conditionEnd == "") {
                    $("#conditionEnd").focus();
                    return;
                }
                else {
        ...
        resize: function () {
            var self = this;
            if (self.jqGrid) {
                self.jqGrid.setGridWidth($("#proStatus").width());
                //grid高度
                var height = 168;
    ...
    var salarySimulationObj = {
        ...
        optionGrid: {
            ...
            colModel: [
                ...
                },
                {
                    ...
                    align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
                        return '<div class="eligibilityCurrentSalary">' + cellvalue + '</div>';
                    }
                },
                {
                    name: 'promoteSalary',
                    ...
                    align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
                        return '<div><input class="eligibilityPromoteSalary" type="text" value="' + Tc.formatFloat(cellvalue) + '" style="text-align:center; font-size:12px; width:60px;"></div>';
                    }
                },
                {
                    name: 'change',
        ...
        init: function () {
            ...
            $("#emulatorModal").modal('show').unbind("shown.bs.modal").on("shown.bs.modal", function () {
            ...
            //提示
            $("#salarySimulation .sstitle i").tooltip({
                placement: 'bottom',
                html: true
            });
                ...
                $(this).on("click", function () {
                    ...
                    if (!b) {
                        self.jqGridUneligibility[i].jqGrid('addRowData', j, {
                            ...
                            promoteSalary: obj.nextranksalary
                        });
        ...
        calculate: function () {
            ...
            $(".eligibilityPromoteSalary").each(function () {
                ...
                obj.find(".eligibilitySalaryTotal").text(value);
            });
            //大计
