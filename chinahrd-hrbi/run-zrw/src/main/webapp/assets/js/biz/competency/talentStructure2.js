require(['jquery', 'echarts', 'echarts/chart/bar', 'echarts/chart/pie', 'echarts/chart/gauge', 'jgGrid', 'bootstrap',
    ...
    var urls = {
        ...
        getManageOrEmpListUrl: webRoot + '/talentStructure/getManageOrEmpList.do',  //职位序列分布 职级分布
    };
    ...
    $(window).resize(function () {
        ...
        managersOrEmpObj.chartObj.resize();
...
//			altclass:'ui-priority-secondary',//用来指定行显示的css，可以编辑自己的css文件，只有当altRows设为 ture时起作用
     ...
     * 管理者员工分布
     * */
    var managersOrEmpObj = {
        chartId: 'managerChart',
        chartObj: initEChart('managerChart'),
        option: optionPie,
        init: function () {
            var self = this;
            if (self.organId == reqOrgId) return;
            self.requestData(reqOrgId);
        },
        requestData: function (organId) {
            var self = this;
            $.post(urls.getManageOrEmpListUrl, {organId: organId}, function (data) {
        ...
        render: function (data) {
            ...
            $.each(data, function (n, v) {
                ...
                if (v != 0) {
                    ...
                    legend.push({name: n, icon: "bar"});
                    i++;
                }
            });

            self.option.series[0].data = lis;
            self.option.legend.data = legend;
            self.chartObj.clear();
            ...
            $("#managerChartTxt").text(rate);
        },
        //管理者员工分布
        rendergGrid: function (dataPie) {
            var self = this, dataTable = [], names = ['下级组织'], modals = [], option = $.extend(true, {}, gridOption);

            $.each(dataPie, function (name, child) {
                var mTotal = 0, eTotal = 0;
                ...
                child['管理者合计'] = mTotal;
                var rate = "";
                if (mTotal > 0 && eTotal > 0) {
                    rate = "1:" + (eTotal / mTotal).toFixed(1);
                } else if (mTotal == 0 || eTotal == 0) {
                    rate = mTotal + ":" + eTotal;
                }
                child['管理者员工比例'] = rate;
            });
