...
//                        rotate: 30
        ...
        render: function () {
            ...
            if(data.list.length > showDataZoomNumber){
            	...
            	self.option.grid = $.extend({}, optionGrid, {y2: 80});
            } else {
    ...
    var salarySubOrgObj = {
        ...
        renderTotal: function () {
            ...
            if(data.length > showDataZoomNumber){
            	self.optionTotal.dataZoom = $.extend({}, dataZoom, {y: 280});
            	self.optionTotal.grid = $.extend({}, optionGrid, {y2: 80});
            } else {
        ...
        renderAvg: function () {
            ...
            setGridX(seriesData, optionGrid);
            if(data.length > showDataZoomNumber){
            	self.optionAvg.dataZoom = $.extend({}, dataZoom, {y: 280});
            	self.optionAvg.grid = $.extend({}, optionGrid, {y2: 80});
            } else {
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function (i, item) {
                        lists.push({
                            ...
                            fixed: Tc.formatFloat(item.fixedSalary).formatMoney(),
                            float: Tc.formatFloat(item.flotSalary).formatMoney()
                        });
                    });
                    self.dataGrid = lists;
                    self.renderGrid();
            ...
            $.get(urls.getSalaryBonusProfitList, {organId: reqOrgId}, function (data) {
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function (i, item) {
                        lists.push({
                            year: item.year + '年',
                            bonus: item.bonus == 0 ? '-' : Tc.formatFloat(item.bonus).formatMoney(),
                            profit: item.profit == 0 ? '-' : Tc.formatFloat(item.profit).formatMoney(),
        ...
        colModel: [
            ...
            {name: 'payDate', width: 130, sortable: false, align: 'center'},
            {
                ...
                formatter: function (cellvalue) {
                    return Tc.formatFloat(cellvalue).formatMoney();
                }
            }
        ],
        rownumbers: true,
        ...
        _.each(yAxis, function (y, e) {
            ...
            _.each(legend, function (l, i) {
                if (i == 0) {    //x轴主数据
                    ...
                    if (xw > 50) {
                        html += '<span class="welfare-bar" style="width: ' + xw + '%;">' + xAxis[e].formatMoney() + '&nbsp;&nbsp;&nbsp;</span>';
                    } else {
                        html += '<span class="welfare-bar" style="width: ' + xw + '%;"></span><span class="welfare-bar-txt">' + xAxis[e].formatMoney() + '</span>';
                    }
                    html += '</span>';
                } else {
