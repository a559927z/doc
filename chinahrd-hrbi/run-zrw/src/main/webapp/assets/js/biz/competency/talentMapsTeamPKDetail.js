require(['jquery', 'jquery-ui', 'bootstrap', 'jgGrid', 'underscore', 'utils', 'messenger', 'talent-map2', 'select2', 'selectItem',
    'organTreeSelector', 'mapGrid', 'inventoryReport', 'keyTalentsAdd', 'jBootstrapPage'], function ($) {
    ...
    var searchBoxObj = {
        ...
        initExistData: function (idx) {
            ...
            if (self.sequenceSubBar && (!_.isNull(param.sequenceSubStr) && !_.isUndefined(param.sequenceSubStr))) self.sequenceSubBar.setSelectItems(param.sequenceSubStr);
        ...
        initSubmitButton: function (bool) {  //初始化按钮事件
            ...
            $submitBtn.unbind('click').click(function () {
                ...
                if (addTeamBool) {  //没有勾选保存则直接渲染地图
                    $.get(urls.getUserTeamNumUrl, function (rs) {
                        if (!rs.type || rs.t >= 3) {
