...
//    			option += "<option value=''>请选择</option>";
        ...
        exportBtnClickFun: function () {
        	var self = this;
        	$("#" + self.exportId).unbind('click').bind('click',function () {
        		exportAttendanceMonthly.init(self.organId);
        	});
        }
    }
...
//            $('#' + self.gridId).setGridHeight($('#' + self.tabId).height() * 0.77);
     ...
     * 导出月考勤excel
     */
    var exportAttendanceMonthly = {
    		modalId:'exportAttendanceMonthlyModal',
    		exportId:'exportAttendanceMonthlyExportingModal',
    		reqUrl: urls.attendanceMonthlyUrl,
    		organId: null,
    		init: function(organId){
    			var self = this;
    			self.organId = organId;
    			self.dateTimecrowdFun();
    			self.setModalShowFun();
    		},
		    dateTimecrowdFun: function () {
		        var self = this;
		        var selectedyearmonth = minMaxTimes[1].substr(0, 4)+'-'+Number(minMaxTimes[1].substr(4, 2));
		        console.log(timeObj.dateEnd);
		        $("#monthSelect").selection({
		            ...
		            dateSelected: [selectedyearmonth],
		            crowdSelected: [],
		            dateSelectedLength: 6,
		            ok: function (event, data) {
		                var cd = data.crowd.join(',') == '0' ? '' : data.crowd.join(',');
		                self.getRequestExcel(self.organId, data.date, cd);
		            }
		        });
		    },
		    getRequestExcel: function(organId, yearmonth,stl){
		    	var self = this;
		    	var form=$("<form>");//定义一个form表单
		    	form.attr("style","display:none");
		    	form.attr("target","");
		    	form.attr("method","post");
		    	form.attr("action",urls.attendanceMonthlyUrl);
		    	var input1=$("<input>");
		    	input1.attr("type","hidden");
		    	input1.attr("name","organId");
		    	input1.attr("value",self.organId);
		    	var input1=$("<input>");
		    	input1.attr("type","hidden");
		    ...
		    setExportShowFun: function () {
		    	var self = this;
		    	$('#'+self.exportId).modal('show');
		    }
