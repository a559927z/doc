 ...
 */
require(['jquery', 'moment', 'echarts', 'echarts/chart/line','echarts/chart/bar','echarts/chart/radar', 'echarts/chart/gauge','bootstrap', 'ace',
    'jquery-pin', 'jgGrid', 'underscore','timeLine2','searchBox3','cardTabel','select2'], function ($, moment, echarts) {
...
//	                        	show:false,
	         	...
	         	init:function (organId,yearMonth) {
	     		    ...
	     		    drawText(cans,"#666666","30px 黑体",(max.rate*100).toFixed(0)+'%',80, 80);
	     		    ...
	     		    $("#high-chart").parent().parent().find(".top-bottom").html(max.positionName)
		      		...
		      		cans.closePath(); 
	     	 		 ...
	     	 		 drawArc(cans,80,80,70,Math.PI,0,'#e0e0e0',15);
	     	 		   ...
	     	 		   drawText(cans,"#666666","30px 黑体",(min.rate*100).toFixed(0)+'%',80, 80);
	        			 ...
	        			 $.ajax({
	        	    	    ...
	        	    	    $("#teamAvgCom_search").unbind("click").click(function(e){
	        	    	    	...
	        	    	    	var val=$("#teamAvg-select").val();
						 		$("#teamAvg-select-showText").html($("#select2-teamAvg-select-container").attr("title"));
	        	        	...
	        	        	var self=this;
	        	        	  $("#teamAvgCom_orgain").children().unbind("click").click(function(e){
		        	       	    	$(this).parent().parent().find("span").html($(this).html())
		        	       	    	var organId=$(this).attr("value");
		        	       	    	$(this).parent().hide();
		        	       	    	self.resetPosition();
		        	       	    	self.reload(organId);
		        	       	    	return false ;
...
//	      			        	                var total = 0;
	  ...
	  var empTabelObj={
		   	...
		   	searchBoxConfig:{//人员明细筛选Option
		             height: 200,
			             attach: [{label: "组织机构", type: "select", data: [{name: "organId", options: []}]},
			                      {
			                 label: "岗位胜任度",
			                 type: "input",
			                 data: [{
		   	...
		   	option:{
		    	   ...
		    	   formatModel:function(el,o,index){
		    		   ...
		    		   $(result).find('.high_div .rate').text(o.high.reallyScoreName); 
		    		   $(result).find('.high_div .name').text(o.high.name); 
		    		   
		    		   $(result).find('.low_div .rate').text(o.low.reallyScoreName); 
		    		   $(result).find('.low_div .name').text(o.low.name); 
		    		   ...
		    		   $(result).find(".competency-card-all").click(function(){
		    			  ...
		    			  $.each(o.list,function(i,obj){
		    				  ...
		    				  seriesData[0].value.push(obj.reallyScore);
		    				  seriesData[1].value.push(obj.expectScore);
		    			  });
		    		   ...
		    		   drawArc(cans,50,50,35,1.5*Math.PI,-0.5*Math.PI,'#e0e0e0',10,1);
			    		 drawArc(cans,50,50,35,1.5*Math.PI,(1.5-o.rate*2)*Math.PI,color,10,1);
			    			
			    		 drawText(cans,color,"25px 黑体",(o.rate*100).toFixed(0)+'%',50, 60); 
		    	   }
		       },
			init:function(organId,time){
				 this.organId=organId;
				 ...
				 //
				  ...
				  $(this.tableId).clearCardTableData().setCardTableParam({
				   ...
				   var params = $.extend(true, {}, this.searchBox.getSelectData(), {organId: this.organId, yearMonth:time});
				   $(this.tableId).clearCardTableData().setCardTableParam({
	                      postData: params
	                  }).reloadCardTable();
	  ...
	  var positionTabelObj={
			   	...
			   	option:{
			    	   ...
			    	   formatModel:function(el,o,index){
			    		   ...
			    		   $(result).find('.high_emp_div .rate').text(o.empList[0].rate*100+"%"); 
			    		   $(result).find('.high_emp_div .name').text(o.empList[0].empName); 
			    		   
			    		   $(result).find('.low_emp_div .rate').text(o.empList[1].rate*100+"%"); 
			    		   $(result).find('.low_emp_div .name').text(o.empList[1].empName); 

			    		   $(result).find('.high_dimension_div .rate').text((o.highDimension.rate*100).toFixed(0)+"%"); 
			    		   $(result).find('.high_dimension_div .name').text(o.highDimension.name); 
			    		   
			    		   $(result).find('.low_dimension_div .rate').text((o.lowDimension.rate*100).toFixed(0)+"%")
			    		   $(result).find('.low_dimension_div .name').text(o.lowDimension.name); 
			    		   ...
			    		   $(result).find(".competency-card-all").click(function(){
			    			   ...
			    			   $("#competency-level-modal").modal('show');
			    			 	 postionCompetencyTabelObj.init(reqOrgId,time,o.positionId);
			    		   });
			    		   return result;
			    	   },loadRowComple:function(el,o,index){
			    		   var color=getColorByCom(o.rate);
				   	...
				   	option:{
				    	   ...
				    	   formatModel:function(el,o,index){
				    		   ...
				    		   cans.scale(2,2);
					    		 ...
					    		 drawText(cans,color,"16px 黑体",(o.rate*100).toFixed(0)+'%',30, 29); 
				    	   },
				       },
...
//				    });;
					 ...
					 $("#guide_position2_btn").click(function(e){
						 var val=$("#guide_position2").val();
					 		...
					 		contrastGuideObj.queryContrastTable();
...
//               rowIds[empId] = rowId;
 	   ...
 	   */
 		...
 		function getColorByCom(val){
           if(val>rangeObj.high[0]){
        	   return "#00b165";
           }else if(val>rangeObj.middle[0]){
        	   return "#F09200";
           }else{
        	   return "#E7191B";
           }
 	  	}
 		function getColorClassByCom(val){
            if(val>rangeObj.high[0]){
         	   return "high";
            }else if(val>rangeObj.middle[0]){
         	   return "middle";
 	  	...
 	  	function setRange(obj){
 	  		rangeObj=obj;
 	  		$("#config_high1").val(obj.high[0]*100);
 	  		$("#config_high2").val(obj.high[1]*100);
 	  		$("#config_middle1").val(obj.middle[0]*100);
 	  		$("#config_middle2").val(obj.middle[1]*100);
 	  		$("#config_low1").val(obj.low[0]*100);
 	  		$("#config_low2").val(obj.low[1]*100);
           topObj.option.series[0].axisLine.lineStyle.color[0] = [obj.low[1], '#e7191b'];
           topObj.option.series[0].axisLine.lineStyle.color[1] = [obj.middle[1], '#F09200'];
...
// 		 		abilityObj.init(reqOrgId,time);
	...
	// 		                    preDistributionObj.resizeAll()
 				 ...
 				 /**
	 		         * 初始化配置事件
	 		         */
	 		  	  $("#tooltip1").find(".index-common-title-right").click(function(){
	 		  	 ...
	 		  	 //基础配置   取消
				 	...
				 	$("#config-modal .btn-primary").click(function(e){
				 		var config_high1=$.trim($("#config_high1").val());
				 		var config_high2=$.trim($("#config_high2").val());
				 		var config_middle1=$.trim($("#config_middle1").val());
				 		var config_middle2=$.trim($("#config_middle2").val());
				 		var config_low1=$.trim($("#config_low1").val());
				 		var config_low2=$.trim($("#config_low2").val());
				 		
				 		if(config_high1!=""&&config_high2!=""
				 			&&config_middle1!=""&&config_middle2!=""
				 				&&config_low1!=""&&config_low2!=""){
				 			var high=[(config_high1/100),(config_high2/100)];
				 			var low=[(config_low1/100),(config_low2/100)];
				 			var middle=[(config_middle1/100),(config_middle2/100)];
				 			setRange({high:high,low:low,middle:middle});
				 			$.ajax({
 	 ...
 	 function initAll(id) {
         ...
         $.ajax({
	  			...
	  			success : function(data){
	  	 		...
	  	 		teamAvgComThrendObj.bindOrganEvent();
