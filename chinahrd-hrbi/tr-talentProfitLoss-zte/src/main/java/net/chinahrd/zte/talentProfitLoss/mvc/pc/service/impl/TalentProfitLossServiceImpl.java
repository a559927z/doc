package net.chinahrd.zte.talentProfitLoss.mvc.pc.service.impl;
...
import net.chinahrd.zte.talentProfitLoss.mvc.pc.dao.TalentProfitLossDao;
...
public class TalentProfitLossServiceImpl implements TalentProfitLossService {
	...
	public Map<String, Object> queryTalentInflowVal(String customerId, String organId, String year, String month,
		...
		List<TalentProfitLossDto> newList = talentProfitLossDao.queryTalentInflowVal(customerId, organId, year, month,
				list);
	...
	public Map<String, Object> queryTalentOutflowVal(String customerId, String organId, String year, String month,
			List<Integer> list) {
		Map<String, Object> map = CollectionKit.newMap();
		List<TalentProfitLossDto> newList = talentProfitLossDao.queryTalentOutflowVal(customerId, organId, year, month,
				list);
		int yearCountNum = 0, monthCountNum = 0;
		String yearCountName = null, monthCountName = null;
		if (newList != null && newList.size() > 0) {
	...
	public List<Object> queryPopulationPie(String customerId, String organId, String provinceName, String[] times,
		...
		parMap.put("customerId", customerId);
		parMap.put("organId", organId);
		parMap.put("provinceName", provinceName);
		parMap.put("date", date);
		parMap.put("list", crowdsList);
	...
	public Map<String, Object> querySequenceBar(String customerId, String organId, String startDate, String endDate,
		...
		parMap.put("maxDate", maxDate);
		parMap.put("list", parentList);
		parMap.put("childType", childType);
		...
		int count = talentProfitLossDao.querySequenceBarCount(parMap);
		List<TalentProfitLossDto> list = talentProfitLossDao.querySequenceBar(parMap);
		if (list != null && list.size() > 0) {
			double per = 0D;
			for (TalentProfitLossDto l : list) {
				if (count > 0) {
					per = formatDouble(l.getConNum() * 100 / count);
				}
	...
	public Map<String, Object> queryAbilityBar(String customerId, String organId, String startDate, String endDate,
		...
		int count = talentProfitLossDao.queryAbilityBarCount(parMap);
		List<TalentProfitLossDto> list = talentProfitLossDao.queryAbilityBar(parMap);
		if (list != null && list.size() > 0) {
			double per = 0D;
			for (TalentProfitLossDto l : list) {
				if (count > 0) {
	...
	public PaginationDto<TalentProfitLossDto> queryEntryListDatas(String customerId, String organId, String startDate,
		...
		String minDate = null, maxDate = null;
		if (startDate != null && !"".equals(startDate)) {
			minDate = startDate + "-01";
		}
		if (endDate != null && !"".equals(endDate)) {
			maxDate = getLastDayForThisMonth(endDate + "-01");
		}
		if (StringUtils.isEmpty(startDate) && StringUtils.isEmpty(endDate)) {
