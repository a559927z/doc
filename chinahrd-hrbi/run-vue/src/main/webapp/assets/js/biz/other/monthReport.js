require(['jquery', 'jquery-ui', 'echarts', 'echarts/chart/bar', 'echarts/chart/line', 'echarts/chart/pie', 'echarts/chart/gauge',
    ...
    var pageSettingObj = {
        ...
        initConfigData: function() {
        	...
        	$.post(urls.getMonthReportConfigUrl, function(data) {
        		self.configData = $.extend(true, [], data);
        		self.tempData = $.extend(true, [], data);
        ...
        initFisrtSetting: function () {
            var self = this, pages = self.pageChilds;
            var $menuBlock = $('#menuBlock'), $fisrtBlock = $('#fisrtSettingBlock');
            ...
            /*$.post(urls.getMonthReportConfigUrl, function(data) {
        		...
        		$.each(data, function(i, obj) {
        			if(obj.flag == 1) {
        				var $this = $(self.panelsId).find('.' + obj.reportPath);
                        $this.addClass('float' + obj.showIndex);
                        $this.find('span.panel-title').text(pageNumberArr[i] + "、" + obj.reportContent);
                      //赋值左边菜单
                        var li = '<li position="float' + (i + 1) + '" class="menu-line menu-line2"><div class="menu-line-spor"></div><span class="menu-line-text">' + obj.reportContent + '</span></li>';
                        if (i == pages.length - 1) {
                            li = '<li position="float' + (i + 1) + '" class="menu-line menu-line3"><div class="menu-line-spor menu-line-spor2"></div><span class="menu-line-text">' + obj.reportContent + '</span></li>'
                        }
                        $menuBlock.append(li);

                        //赋值弹出框
                        $fisrtBlock.append('<div class="setting-page">' + obj.reportContent + '</div>');
            ...
            $.each(pages, function (i, obj) {
                var $this = $(obj);
                var idx = $this.data('action');
                $this.addClass('float' + (i + 1));
                var pageName = pageNameArr[idx - 1];
                $this.find('span.panel-title').text(pageNumberArr[i] + "、" + pageName);
                //赋值左边菜单
                var li = '<li position="float' + (i + 1) + '" class="menu-line menu-line2"><div class="menu-line-spor"></div><span class="menu-line-text">' + pageName + '</span></li>';
                if (i == pages.length - 1) {
                    li = '<li position="float' + (i + 1) + '" class="menu-line menu-line3"><div class="menu-line-spor menu-line-spor2"></div><span class="menu-line-text">' + pageName + '</span></li>'
                }
                $menuBlock.append(li);

                //赋值弹出框
                $fisrtBlock.append('<div class="setting-page">' + pageName + '</div>');
            });
            ...
            //第一次弹出框选中事件
            $fisrtBlock.children('.setting-page').unbind('click').bind('click', function() {
                $(this).toggleClass('selected');
            });

            //第一次弹出框确认事件
            $fisrtBlock.find('.success-btn').unbind('click').bind('click', function() {
    ...
    $(window).scroll(function (e) {
        ...
        } else if (top >= $(".float6").offset().top - floatNumber && top < $(".float7").offset().top - floatNumber) {
            $(".menu-line").removeClass("menu-line-select");
            $(".menu-line").eq(6).addClass("menu-line-select");
        } else if (top >= $(".float7").offset().top - floatNumber && top < $(".float8").offset().top - floatNumber) {
            $(".menu-line").removeClass("menu-line-select");
            $(".menu-line").eq(7).addClass("menu-line-select");
        } else if (top >= $(".float8").offset().top - floatNumber && top < $(".float9").offset().top - floatNumber) {
            $(".menu-line").removeClass("menu-line-select");
            $(".menu-line").eq(9).addClass("menu-line-select");
        } else if (top >= $(".float9").offset().top - floatNumber && top < $(".float10").offset().top - floatNumber) {
            $(".menu-line").removeClass("menu-line-select");
            $(".menu-line").eq(10).addClass("menu-line-select");
        }
    });
...
//			canvas.style.cssText += canvasStyle;
    ...
    var promotionObj = {
        ...
        requestInJobsData: function (organId) {
            ...
            var param = {
            	organId: organId,
            	yearMonth: '201507'//pageSettingObj.yearMonth
            }
