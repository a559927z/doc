...
define(['jquery','echarts','echarts/chart/scatter','bootstrap','messenger'], function ($,echarts) {
	...
	var head= '<div class="row base-info">'+
					'<div class="col-xs-12 ">'+
						'<div class="row">'+
							'<!-- 左边头像 -->'+
							'<div class="col-xs-2 ">'+
...
//											'</div>'+
				    ...
				    '</div><!-- /.modal -->'+
				'</div>';
	var template=  '<div class="row riskTree ">'+head+
				 '<div class="row col-xs-12 detail-info">'+
			           '<div class="col-xs-6">'+
						//	'<div class="row">'+
								'<div class="col-xs-11 lastTime">最新离职风险评估</div>'+
								'<div class="col-xs-11 risk-detail-info">'+
									'<div class="risk-tree-div"><div></div></div>'+
//									'<div class="risk-memo">'+
//									'</div>'+
...
//        		});
        ...
        _createSubDom : function(){
        	...
        	var self=this;
        	var detailText = container.find('.base-info .row').last().find('span');
        	var lastTime=container.find(".lastTime");
        	var empObj=self._options.empObj;
        	 detailText.eq(0).text(empObj.userNameCh);
        	 detailText.eq(1).text( empObj.sequenceName + ' ' + empObj.abilityName);
...
//             var flagClass = 'risk-flag img-circle ' + flagItemArr[empObj.riskFlag||0].color;
//             container.find('.base-info .row').last().find('span').eq(1).removeClass().addClass(flagClass);
