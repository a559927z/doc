...
require(['jquery','bootstrap', 'underscore', 'utils','resize'], function ($, echarts) {
    ...
    var incentiveFactorsLabel = {
        ...
        bindUpdateLableClickEvent: function () {
            var self = this;
            $('.incentiveFactors').find('.labelRight').unbind("click").click(function () {
    ...
    var trackingLogLabel = {
        initData: function () {
            var self = this;
            var trackingLogTable = $('.trackingLog').find('table');
            var src = webRoot + "/assets/photo.jpg";
            var logContent = [];
            var flag = false;
                ...
                $.each(result, function (i, item) {
                    ...
                    if (i <= 2) {
                        ...
                        if (customerId == item.createEmpId) {
                            ...
                            tr3 = '<tr><td align="right">最近更新时间：' + createTime + '</td><br/></tr>';
                        }
                        trackingLogTable.append(tr1);
                        trackingLogTable.append(tr2);
                        trackingLogTable.append(tr3);
                    } else {
        ...
        trackingLogLabelClickEvent: function () {
            var self = this;
            $('.trackingLog').find('.labelRight').unbind('click').click(function () {
                $('#logLabelDetailModal').modal('show');
