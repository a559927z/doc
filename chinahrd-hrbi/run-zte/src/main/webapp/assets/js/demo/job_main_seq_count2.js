require(['utils','organTreeSelector','jgGrid'], function(dp) {
	...
	var jobMainSeqCount = {
		...
		_detailseqShowCall:function(mainSeqId){
			...
			$(".detailseq_area").show(0,function(){
				...
				$('#chart1').text('发ajax请求，返回数据后生成图表。'+ mainSeqId).css({'text-align':'center','line-height':'300px'});
				$('#chart2').text('发ajax请求，返回数据后生成图表。'+mainSeqId).css({'text-align':'center','line-height':'300px'});
				$('#chart3').text('发ajax请求，返回数据后生成表格。机构ID：'+unitId + '表格ID：'+mainSeqId).css({'text-align':'center','line-height':'250px'});
