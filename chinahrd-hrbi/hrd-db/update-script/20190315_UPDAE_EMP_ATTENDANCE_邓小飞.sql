...
-- ----------------------------------------------------
-- 2月份考勤中对应的1天，02日(正常出勤 --> 年假)
-- ----------------------------------------------------
call pro_update_emp_attendance('1','邓小飞','正常出勤','年假','2019-02-01','2019-02-03', -1.0);
-- 添加年假
INSERT INTO emp_vacation (emp_id, customer_id, emp_key, user_name_ch, annual, refresh)VALUE
(
  (SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '邓小飞' ),
  "1",
  (SELECT user_key FROM dim_user t WHERE t.user_name_ch = '邓小飞' ),
  "邓小飞",
  "-1.0",
  CURDATE()
);
