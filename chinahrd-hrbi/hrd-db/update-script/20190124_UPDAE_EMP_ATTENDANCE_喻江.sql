...
BEGIN
	...
	CALL pro_fetch_emp_attendance_month ( "1", "SYSTEM", (SELECT @session.empId), 201812);
...
-- ----------------------------------------------------
-- 12月份考勤中对应的2天，13日 14日 17日 18日(正常出勤 --> 年假)
-- ----------------------------------------------------
call pro_update_emp_attendance('1','喻江','正常出勤','年假','2018-12-13','2018-12-14', -2.0);
call pro_update_emp_attendance('1','喻江','正常出勤','年假','2018-12-17','2018-12-18', -2.0);
UPDATE emp_vacation SET annual_total = 5, annual = 5 WHERE emp_id = (SELECT @session.empId);
