...
-- ----------------------------------------------------
-- 2月份考勤中对应的3天，03,11~12日(正常出勤 --> 年假)
-- ----------------------------------------------------
call pro_update_emp_attendance('1','罗俊杰','正常出勤','年假','2019-02-03','2019-02-03', -1.0);
call pro_update_emp_attendance('1','罗俊杰','正常出勤','年假','2019-02-11','2019-02-12', -2.0);
-- 添加年假
INSERT INTO emp_vacation (emp_id, customer_id, emp_key, user_name_ch, annual, refresh)VALUE
(
  (SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '罗俊杰' ),
  "1",
  (SELECT user_key FROM dim_user t WHERE t.user_name_ch = '罗俊杰' ),
  "罗俊杰",
  "-3.0",
  CURDATE()
);
