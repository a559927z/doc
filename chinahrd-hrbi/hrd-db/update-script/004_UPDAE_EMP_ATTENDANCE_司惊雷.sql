-- ----------------------------------------------------
-- 事假变更为预支年假，时间为11日，12日，13日，14日，22日，共5天
-- ----------------------------------------------------
...
WHERE emp_attendance_id IN 
	(
		SELECT * FROM (
			...
			WHERE
				t.emp_id = ( SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '司惊雷' )
...
UPDATE emp_attendance_day SET
	checkwork_type_id = (SELECT checkwork_type_id FROM dim_checkwork_type t WHERE t.checkwork_type_name='年假')
		WHERE		emp_id = ( SELECT emp_id FROM dim_user t WHERE user_name_ch = '司惊雷' )
		AND days in ('2018-02-11','2018-02-12','2018-02-13','2018-02-14','2018-02-22')
;
-- emp_attendance_month
CALL pro_fetch_emp_attendance_month ( "1", "SYSTEM", ( SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '司惊雷' ), 201802 );
...
WHERE emp_other_day_id IN
	(
		SELECT emp_other_day_id FROM (
			...
			WHERE
				t.emp_id = ( SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '司惊雷' )
			AND t.days in ('2018-02-11','2018-02-12','2018-02-13','2018-02-14','2018-02-22')
...
(
  (SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '司惊雷' ),
  "1",
  (SELECT user_key FROM dim_user t WHERE t.user_name_ch = '司惊雷' ),
  "司惊雷",
  "-5.0",
  CURDATE()
)
