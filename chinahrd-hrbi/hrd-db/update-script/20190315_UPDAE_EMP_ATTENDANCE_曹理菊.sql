...
-- ----------------------------------------------------
-- 2月份考勤中对应的4天，01~03, 12日(正常出勤 --> 年假)
-- ----------------------------------------------------
call pro_update_emp_attendance('1','曹理菊','正常出勤','年假','2019-02-01','2019-02-03', -3.0);
call pro_update_emp_attendance('1','曹理菊','正常出勤','年假','2019-02-12','2019-02-12', -1.0);
-- 添加年假
INSERT INTO emp_vacation (emp_id, customer_id, emp_key, user_name_ch, annual, refresh)VALUE
(
  (SELECT emp_id FROM dim_user t WHERE t.user_name_ch = '曹理菊' ),
  "1",
  (SELECT user_key FROM dim_user t WHERE t.user_name_ch = '曹理菊' ),
  "曹理菊",
  "-4.0",
  CURDATE()
);
