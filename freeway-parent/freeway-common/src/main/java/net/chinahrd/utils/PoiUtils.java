...
public class PoiUtils {
    private static String rootPath = "d:/file_tmp/";
    
    public static void main(String[] args) {
        String fileName = "test2html.docx";
        officeToHtml(fileName);
    }
    
    
    public static void officeToHtml(String fileName) {
        officeToHtml(rootPath,fileName);
    }
     ...
     */
    public static boolean officeToHtml(String filePath,String fileName) {
        fileName = filePath + fileName;
        String htmlFilePath = fileName.substring(0,fileName.indexOf("."))+".html";
        try {
            if(checkFile(fileName,"doc")){
                return wordToHtml03(fileName,htmlFilePath);
            }else if(checkFile(fileName,"docx")){
                return wordToHtml07(fileName,htmlFilePath);
            }else if(checkFile(fileName,"ppt")){
                return pptToHtml03(fileName,htmlFilePath);
            }else if(checkFile(fileName,"pptx")){
                return pptToHtml07(fileName,htmlFilePath);
            }else {
...
//                        pic.writeImageContent (new FileOutputStream (path + pic.suggestFullFileName() ) );
    ...
    public static boolean pptToHtml03(String filepath, String outputFile){
        ...
        if (!checkFile(filepath,"ppt")) {
            log.error("ppt03文件转html出错，不支持类型为："+filepath.substring(filepath.indexOf("."))+" 的文件");
            return false;
        }
    ...
    public static boolean pptToHtml07(String filepath,String outputFile) {
        ...
        if (!checkFile(filepath,"pptx")) {
            log.error("ppt07文件转html出错，不支持类型为："+filepath.substring(filepath.indexOf("."))+" 的文件");
            return false;
        }
        FileInputStream is = null;
        SlideShow ppt;
    ...
    private static String getImageUrl(String filePath){
        return rootPath+filePath.substring(0,filePath.indexOf("."))+"/";
    }

    private static String getImageSavePath(String filePath){
        return filePath.substring(0,filePath.indexOf("."))+File.separator;
    }
