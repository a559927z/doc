package net.chinahrd.platform.rbac.filter;
                ...
                if (rbacUserRes != null) {
                    ...
                    if (!CollectionUtils.isEmpty(roles)) {
                        for (RbacRoleRes rbacRoleRes : roles) {//角色
                            List<RbacResourceRes> rbacResourceRes = rbacRoleRes.getResources();
                            if (!CollectionUtils.isEmpty(rbacResourceRes)) {
                                for (RbacResourceRes rbacModuleRes : rbacResourceRes) {//权限
                                    if (StringUtils.equalsIgnoreCase(requestURL, rbacModuleRes.getResourceUrl())) {
                                        return true;
                                    }
                                }
                            }
